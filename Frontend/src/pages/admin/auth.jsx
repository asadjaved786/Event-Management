  ...
  const handleSubmit = async (event) => {
    ...
    if (response.status === 200) {
      if (data?.user?.role == "admin") {
        ...
        setMessage({ errorMsg: "Forbidden", successMsg: "" });
      }

      //   setAdminToken(data.admin_token); // set cookie when signed up
    } else {
      console.error(`Failed with status code ${response.status}`);
      setMessage({ errorMsg: data.message, successMsg: "" });
    }
  };
            ...
            step === 1 && (
              <form onSubmit={handleSubmit}>
                ...
                <p className="text-sm text-gray-700 mt-6">
                  *You have the option to designate yourself as an admin for
                  testing purposes by following this{" "}
                  <a
                    href="/"
                    target="_blank"
